...
const registerUser = async (req, res) => {
  try {
    ...
    const { firstname,lastname,username, email, phone, password,city,state,zip } = req.body;
    //check id user with provided email or phone already exists
    const existingUser = await UserModel.findOne({ email });
    if(existingUser){
        return res.status(httpStatusCode.CONFLICT).json({
            success:false,
            message:"User is already registeres with this email or phone. please sign in",
        });
    }
